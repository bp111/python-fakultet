...
def train_logistic_regression(X_train, y_train, cancel_event):
    ...
    yield f"Best LR Parameters found: {grid.best_params_}\n"
    yield ("MODEL", grid.best_estimator_)

def train_knn(X_train, y_train, cancel_event):
    ...
    yield f"Best KNN Parameters found: {grid.best_params_}\n"
    yield ("MODEL", grid.best_estimator_)

def train_svm(X_train, y_train, cancel_event):
    ...
    yield f"Best SVM Parameters found: {grid.best_params_}\n"
    yield ("MODEL", grid.best_estimator_)

def train_random_forest(X_train, y_train, cancel_event):
    ...
    yield f"Best Random Forest Parameters found: {grid.best_params_}\n"
    yield ("MODEL", grid.best_estimator_)

def train_neural_network(X_train, y_train, cancel_event):
    ...
    yield f"Best Neural Network Parameters found: {grid.best_params_}\n"
    yield ("MODEL", grid.best_estimator_)